export function findSimilarListings(listings, listing, limit = 10) {
  ...
  return listings
    ...
    .filter((candidate) => String(candidate.locality || "").trim().toLowerCase() === String(listing.locality || "").trim().toLowerCase())
    .filter((candidate) => Number(candidate.bedroom) === Number(listing.bedroom))
